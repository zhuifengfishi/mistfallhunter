...
export default async function GuidePage({ params }: GuidePageProps) {
  ...
  return <article className="guide-page">
    ...
    <section className="guide-related"><p className="home-kicker">Keep reading</p><h2>Related Mistfall Hunter guides</h2><div>{guide.related.map((related) => { const item = getGuide(related); return item && <a href={localizedPath(locale, `/guides/${item.slug}`)} key={item.slug}>{item.title}<span aria-hidden="true">→</span></a>; })}</div></section>
